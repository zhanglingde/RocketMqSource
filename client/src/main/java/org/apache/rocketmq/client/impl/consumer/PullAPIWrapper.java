...
public class PullAPIWrapper {
     ...
     * 2. 解析消息，并根据订阅信息消息 tagCode 匹配适合消息
     *
     * @param mq 消息队列
     ...
     * @return 拉取消息结果。只有通讯模式为同步时，才返回结果，否则返回 null
     * @throws MQClientException 当寻找不到 broker 或发生其他 client 异常
     * @throws RemotingException 远程调用发生异常
    ...
    public PullResult pullKernelImpl(final MessageQueue mq,
        ...
        if (null == findBrokerResult) {
            // Broker 为空从 Namesrv 更新 Broker 到本地
        ...
        if (findBrokerResult != null) {
            ...
            // 设置 Broker 最长阻塞时间，默认是 15s（在 Broker 没有新消息时才阻塞，有消息时会立刻返回）
